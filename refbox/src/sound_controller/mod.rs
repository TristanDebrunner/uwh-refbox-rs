...
#[cfg(target_os = "linux")]
const BUTTON_PUSH_DUR: Duration = Duration::from_secs(2);
...
impl SoundController {
            ...
            + 'static,
    {
        ...
        #[cfg(target_os = "linux")]
        let mut wired_tx = if let Ok(sys_info) = rppal::system::DeviceInfo::new() {
            ...
            let mut wired_pin = gpio.get(12).unwrap().into_output();
            let (wired_tx, mut wired_rx) = unbounded_channel();
            // wired_pin
            //     .set_async_interrupt(Trigger::Both, move |level| wired_tx.send(level).unwrap())
            //     .unwrap();

            let button_writer = task::spawn(async move {
                loop {
                    match wired_rx.recv().await {
                        Some(_) => {
                            wired_pin.set_high();
                            sleep(BUTTON_PUSH_DUR).await;
                            wired_pin.set_low();
                            'inner: loop {
                                match wired_rx.try_recv() {
                                    Ok(_) => {}
                                    Err(_) => break 'inner,
                                }
                            }
                        }
                        None => panic!("Won't get any more messages"),
                    }
                }
            });

            tasks.push(button_writer);
                            ...
                            Some(msg) => {
                                ...
                                match msg {
                                    SoundMessage::TriggerBuzzer => {
                                        ...
                                        #[cfg(target_os = "linux")]
                                        if let Some(tx) = wired_tx.as_mut() {
                                            tx.send(true).unwrap();
                                        }
    ...
    pub fn request_next_remote_id<F>(&self, _callback: F)
    where
